...
exports.validateFormAsync = validateFormAsync;
 ...
 */
function evaluateCondition(condition, formData, rowData) {
    let fieldValue;
 ...
 */
function validateRule(value, rule, fieldConfig, formData, rowData) {
    ...
    switch (rule.type) {
        ...
        case 'email':
            if (isEmpty(value))
                return true; // Don't validate empty (use required for that)
        ...
        case 'custom':
            if (rule.customValidatorName) {
                const customValidator = registry_js_1.validatorRegistry.get(rule.customValidatorName);
                if (customValidator) {
 ...
 */
function validateField(field, value, formData, fieldPath = field.name) {
    const errors = [];
 ...
 */
function validateTableField(field, tableData, formData) {
    const errors = [];
    ...
    tableData.forEach((row, rowIndex) => {
        ...
        for (const column of config.columns) {
            ...
            for (const rule of column.validations) {
                const isValid = validateRule(cellValue, rule, { ...field, name: column.name }, formData, row // Pass row data for condition evaluation
                );
                if (!isValid) {
                    errors.push({
                        field: cellPath,
 ...
 */
function validateDataGridField(field, gridData, formData) {
    const errors = [];
    ...
    for (const rowLabel of config.rowLabels) {
        ...
        for (const column of config.columns) {
            ...
            for (const rule of column.validations) {
                const isValid = validateRule(cellValue, rule, { ...field, name: column.name }, formData, rowData // Pass row data for condition evaluation
                );
                if (!isValid) {
                    errors.push({
                        field: cellPath,
 ...
 */
function validatePhoneField(field, phoneData, formData) {
    const errors = [];
    ...
    if (hasRequired) {
        const isPhoneEmpty = !phoneData || (!phoneData.number && !phoneData.countryCode) || !phoneData.number?.trim();
        if (isPhoneEmpty) {
 ...
 */
function validateDateRangeField(field, dateRangeData, formData) {
    const errors = [];
    if (!field.validations)
        return errors;
 ...
 */
function validateFieldValue(fieldConfig, value, formData = {}) {
 ...
 * ```
 */
async function validateFieldAsync(fieldConfig, value, formData = {}) {
    const asyncConfig = fieldConfig.asyncValidation;
    if (!asyncConfig || !asyncConfig.validatorName) {
        return { valid: true };
    }
    const validator = registry_js_1.asyncValidatorRegistry.get(asyncConfig.validatorName);
    if (!validator) {
 ...
 * ```
 */
async function validateFormAsync(config, data) {
    const errors = [];
    ...
    for (const field of config.fields) {
        ...
        if (!field.asyncValidation || !field.asyncValidation.validatorName)
            continue;
        // Skip fields where validator is not registered
        if (!registry_js_1.asyncValidatorRegistry.has(field.asyncValidation.validatorName)) {
